...
/* #define CONFIG_USE_XIN_FOR_I2S 1 */
//#define I2S_XIN_RATE 	(49152 *KHZ)
#define I2S_XIN_RATE 	(24576 *KHZ)
...
#if 0//def CONFIG_USE_XIN_FOR_I2S
		if (pll_id == CPLL_ID)
				return I2S_XIN_RATE;
#endif
		rate = parent_rate;
...
static int plus_cpll_clk_set_rate(struct clk *c, unsigned long rate)
{
    ...
    printk( KERN_ERR "zzdts clock plus_pll_clk_set_rate %d",__LINE__);
    WARN_ON(1);
#ifdef CONFIG_USE_XIN_FOR_I2S
	/* if (rate == I2S_XIN_RATE) { */
	if (rate == 48000*512 || rate == 44100*512 ) {
		printk("use cpll as i2s_xin\n");
		cru_writel(PLL_MODE_DEEP(pll_data->id), CRU_MODE_CON);
		cru_writel((0x1 << (16+1)) | (0x1<<1), PLL_CONS(pll_data->id, 3));
		return 0;
	}
#endif
...
void rk30_clock_common_i2s_init(void)
{
	...
	clk_set_rate_nolock(&clk_i2s0, codec_pll_clk.rate);
	return;
#endif
	//20 times
...
void __init _rk30_clock_data_init(unsigned long gpll, unsigned long cpll, int flags)
{
	...
	if (soc_is_rk3188plus()) {// 3188T SOC
		printk("%s soc_is_rk3188plus", __func__);

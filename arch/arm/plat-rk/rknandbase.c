...
char GetNBSectorInfoBeforeNandInit(char * pbuf)
{
    memcpy(pbuf,&nand_idb_data[0x620],0x200);
    return 0;
}
...
static struct proc_dir_entry *nb_sector_proc_entry;
...
static void rknand_create_sector_procfs(void)
{
    ...
    /* Install the proc_fs entry */
    nb_sector_proc_entry = create_proc_entry("rknand_sector_nb",
                           S_IRUGO | S_IFREG,
                           NULL);

    if (nb_sector_proc_entry) {
        nb_sector_proc_entry->write_proc = NULL;
        nb_sector_proc_entry->read_proc = rkNand_sector_proc_read;
        nb_sector_proc_entry->data = kzalloc(RKNAND_SECTOR_SIZE, GFP_KERNEL);
        if(nb_sector_proc_entry->data)
            GetNBSectorInfoBeforeNandInit(nb_sector_proc_entry->data);
    }
}

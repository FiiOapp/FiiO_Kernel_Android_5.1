...
noinline void ddrfreq_set_sys_status(enum SYS_STATUS status)
{

	dprintk(DEBUG_DDR, "set status %02x\n",status);
	set_bit(status, &ddr.sys_status);
	wake_up(&ddr.wait);
}

noinline void ddrfreq_clear_sys_status(enum SYS_STATUS status)
{
	dprintk(DEBUG_DDR, "clear status %02x\n",status);
	clear_bit(status, &ddr.sys_status);
	wake_up(&ddr.wait);
}

static ssize_t ddr_status_store(struct device *dev,
                                ...
                                const char *buf, size_t size)
{
    ...
    if(sscanf(buf,"%s 0x%x\n",str1,&status)==2){

        dprintk(DEBUG_DDR, "DDR status get 0x%x\n",status);
    }else if(sscanf(buf,"%s %s\n",str1,str2)==2){

        dprintk(DEBUG_DDR, "DDR status get %s\n",str2);
        if(!strcmp(str2,"audio")){

            status = SYS_STATUS_AUDIO;

        }else if(!strcmp(str2,"usb")){

            status = SYS_STATUS_USB;

        }else{

            dprintk(DEBUG_DDR, "DDR status get %s not support!!\n",str2);
            return size;
        }
    }else{

        dprintk(DEBUG_DDR, "DDR cmd not support\n");
    }
    ...
    if(status >=SYS_STATUS_MAX){

        dprintk(DEBUG_DDR, "status %d is not support\n",status);
        return size;
    }

    if(!strcmp(str1,"set")){
        ddrfreq_set_sys_status(status);
    }else if(!strcmp(str1,"clr")){
...
static DEVICE_ATTR(ddr_status, S_IALLUGO, ddr_status_show, ddr_status_store);
static struct attribute *sysfs_attrs_ctrl[] = {
    &dev_attr_ddr_status.attr,
    NULL
};
...
static noinline void ddrfreq_work(unsigned long sys_status)
{
	...
	} else if (ddr.normal_rate && (s & (1 << SYS_STATUS_AUDIO))) {
		ddrfreq_mode(false, &ddr.normal_rate, "audio");
	} else if (ddr.normal_rate && (s & (1 << SYS_STATUS_USB))) {
		ddrfreq_mode(false, &ddr.normal_rate, "USB");

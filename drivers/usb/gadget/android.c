...
extern void charge_set_param(int vol,int cur);
#endif
...
static void android_work(struct work_struct *data)
{
	...
	if (uevent_envp) {
        ...
        #if defined(CONFIG_LIDA_MACH_X7)
                if(!strcmp(uevent_envp[0], configured[0]))
                {
                        axp_usbcur();
                        axp_usbvol();
                }
        ...
        #endif
        #if defined(CONFIG_LIDA_MACH_X5)
            #warning "android usb envent"
                if(!strcmp(uevent_envp[0], configured[0])){
                    // usb connected to computer
                        /* charge_set_param(); */
                        /* printk(KERN_ERR "%s usb connected to computer\n",__func__); */

                }else if(!strcmp(uevent_envp[0], connected[0])){
                    // usb connected to charge adapter
                        /* printk(KERN_ERR "%s set usb connected to charger\n",__func__); */
                        /* usb_path_selecter(1);// connected to charge die */

                } else{
                    // usb disconnected 
                        /* printk(KERN_ERR "%s usb disconnected,set usb connected to soc \n",__func__); */
                        /* usb_path_selecter(0);// connected to soc */
...
static int audio_function_unbind_config(struct android_usb_function *f,
						struct usb_configuration *c)
{
        printk("%s:  enter  !!!\n", __FUNCTION__);
#ifdef UAC2_CONFIG
...
static ssize_t audio_pcm_rate_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct android_usb_function *f = dev_get_drvdata(dev);

	/* print PCM sample rate */
	return sprintf(buf, "%d\n", actual_rate);
}

static ssize_t uac_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct android_usb_function *f = dev_get_drvdata(dev);

	/* print USB Audio Class enable flags */
	return sprintf(buf, "%d\n", uac_flags);
}

static ssize_t uac_flag_store(struct device *dev, struct device_attribute *attr,
			    const char *buff, size_t size)
{
	...
	if (enabled && !uac_flags) {
		uac_flags = true;
	} else if (!enabled && uac_flags) {
		uac_flags = false;
	} else {
		pr_err("usb_audio: already %s\n",
				uac_flags ? "enabled" : "disabled");
	}

	return size;
}

static DEVICE_ATTR(sample_rate, S_IRUGO | S_IWUSR, audio_pcm_rate_show, NULL);
static DEVICE_ATTR(uac_flag, 0664, uac_flag_show, uac_flag_store);

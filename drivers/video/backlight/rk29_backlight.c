...
static int power_light_status = 0;
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (count == size) {
		ret = count;
                power_light_status = status;
                DBG("power_light status: %d\n", status);

                reset_power_light_ctrl(power_light_status);
	}

	return ret;
}

static ssize_t power_light_read(struct device *dev,
		struct device_attribute *attr, char *buf)
{
        return sprintf(buf, "%d\n", power_light_status);
}
static DEVICE_ATTR(power_light, 0660, power_light_read, power_light_write);
#endif
...
#if defined(CONFIG_LIDA_MACH_X5)
	ret = device_create_file(&pdev->dev, &dev_attr_power_light);
	if(ret)
	{
		dev_err(&pdev->dev, "failed to create power_light sysfs file\n");
	}
#endif

...
int act8846_set_bits(struct act8846 *act8846, u8 reg, u16 mask, u16 val);
...
int act8846_sdcard_power(char on)
{
    struct regulator *ldo;

    ldo = regulator_get(NULL, "act_ldo4");
    if(ldo == NULL || IS_ERR(ldo)) {
        printk(KERN_ERR "sdcard power get error \n");
        return -1;
    } else {
        if(on) {
            if(regulator_is_enabled(ldo) == 0) {
                ...
                printk("sdcard power on 3V3\n");
            } else {
                printk("sdcard power on 3V3 have opened\n");
            }
        } else {
            if(regulator_is_enabled(ldo) >= 1){
                regulator_disable(ldo);
                printk("sdcard power off 3V3\n");
            }else{
                printk("sdcard power off 3V3 have closed\n");
            }
        }

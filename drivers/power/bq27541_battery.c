...
//		gpio_set_value(POWER_ON_PIN, GPIO_LOW);
	...
	if((volt <= 3400000)  && (volt > 0) /*&& gpio_get_value(di->dc_check_pin)*/){
        low_vol_count+=1;
		printk("vol smaller then 3.4V %d\n",volt); 
    }else{
        low_vol_count=0;
    }
...
static int bq27541_battery_voltage_ocv(struct bq27541_device_info *di)
{
	int ret;
	u8 buf[2] = {0};
	int volt = 0;

	ret = bq27541_read(di->client, 0x10,buf,2); 
	if (ret<0) {
		dev_err(di->dev, "error reading voltage ocv\n");
		return ret;
	}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
        ...
        case POWER_SUPPLY_PROP_CAPACITY_OCV:
                val->intval = bq27541_battery_voltage_ocv(di);
                break;
...
static int bq27541_i2c_test(struct i2c_client *client)
{
        char buf[2];
        int ret;
        int count = 5;
        ...
        do {
	    ret = bq27541_read(client, BQ27x00_REG_FLAGS, buf, 2);
	    if (ret < 0) {
...
int bq27541_read_reg(u8 reg)
{
        ...
        di = bq27541_di;
        bq27541_read(di->client, reg, buf, 2);
        ret = get_unaligned_le16(buf);
        
        return ret;
}
EXPORT_SYMBOL_GPL(bq27541_read_reg);
				 ...
				 const struct i2c_device_id *id)
{
	...
	pdata = client->dev.platform_data;
        ...
        retval = bq27541_i2c_test(client);
        if (retval < 0) {
            dev_err(&client->dev, "%s:  error !!!\n", __func__);
            return retval;
        }
	...
	dev_info(&client->dev, "support ver. %s enabled\n", DRIVER_VERSION);
        ...
        is_bq27541_flag = true;

#if  0//!defined (CONFIG_NO_BATTERY_IC)

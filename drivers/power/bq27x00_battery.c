...
#define dprintk(level, fmt, arg...) do {			\
	if (debug >= level) 					\
	printk(KERN_WARNING "[%s]-[%04d]: " fmt ,__FUNCTION__, __LINE__, ## arg); } while (0)
...
static struct dm_reg bq274xx_dm_regs[] = {
	...
	{82, 10, 2, 3640},	/* Design Capacity */
	{82, 12, 2, 3700},	/* Design Energy */
	{82, 16, 2, 3450},	/* Terminate Voltage */
...
static void rom_mode_gauge_dm_init(struct bq27x00_device_info *di)
{
    ...
    CODEC_DG("bq27x00 status 0x%x \n",control_cmd_read(di,CONTROL_STATUS_SUBCMD));
...
static void bq27x00_battery_poll(struct work_struct *work)
{
    ...
    CODEC_DG(" poll_interval %d\n",poll_interval);
	if (poll_interval > 0) {
		/* The timer does not have to be accurate. */
    CODEC_DG(" schedule_delayed_work\n");

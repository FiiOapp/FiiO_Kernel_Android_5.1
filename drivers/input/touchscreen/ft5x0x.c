...
static void ft5x0x_power_en(struct ft5x0x_data *tsdata, int on)
{
	...
	if (on) {
       ...
       if( tsdata->reset_gpio <= 0 ){
               ...
               vcc_tp = regulator_get(NULL, VCC_NAME);   // vcc28_cif
...
static int ft5x0x_chip_init(struct i2c_client * client)
{
	...
	mdelay(500);
        ret = ft_cmd_write(0x07,0x00,0x00,0x00,1);
	mdelay(10);
        if (ret < 0) {
            printk("%s: ft_cmd_write failed !!!\n", __func__);
            return ret;
        }
...
static int ft5x0x_process_points(struct ft5x0x_data *data)
{
    ...
    for (i = 0; i < touch_num; i++) {        
		...
		if (x < (SCREEN_MAX_X + 10)) {
            ...
            #ifdef INVERT_Y
				input_report_abs(data->input_dev, ABS_MT_POSITION_Y, 800-y);
            #else
...
static int  ft5x0x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    i=ft5x0x_read_reg(client,FTS_REG_FW_VER);
    printk("CTP Upgrade %s  new version: 0x%x \n",!i? "Success":"Failed",i);
...
subsys_initcall_sync(ft5x0x_mod_init);

...
static int vtl_ts_config(struct ts_info *ts)
{
	...
	int val;
	struct ct36x_platform_data *pdata = &ts->driver->client->dev.platform_data;
	...
	{	
		...
		printk("the rst is %d , irq is %d \n",ts->config_info.rst_gpio_number, ts->config_info.irq_gpio_number);
	}
	else
	{
		printk("config err,so return -1!");
		return -1;
	}
...
int vtl_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->config_info.screen_max_x = pdata->x_max;
	ts->config_info.screen_max_y = pdata->y_max;
	ts->config_info.irq_gpio_number = pdata->irq_io.gpio;
	ts->config_info.rst_gpio_number = pdata->rst_io.gpio;
...
*/
static const struct i2c_device_id vtl_ts_id_gy[] = {
	{ DRIVER_NAME, 0 },
	{ }
};

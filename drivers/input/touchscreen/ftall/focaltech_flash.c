...
static unsigned char CTPM_FW[] = {
	...
	#include "FT6336U_X5III_V16_app.i"
#elif defined(CONFIG_LIDA_MACH_X7II)
	#include "FT5346_X7II_V01_app.i"
#else
    #error "Please define ft upgrade fw here"
#endif
};

static unsigned char aucFW_PRAM_BOOT[] = {
	/* #include "FT6336U_X5III_V11_app.i" */
};
...
int hidi2c_to_stdi2c(struct i2c_client * client)
{
	...
	fts_flash_i2c_read(client, auc_i2c_write_buf, 0, auc_i2c_write_buf, 3);
...
void fts_update_fw_vendor_id(struct fts_ts_data *data)
{
	...
	err = fts_flash_i2c_read(client, &reg_addr, 1, &data->fw_vendor_id, 1);
	if (err < 0)
...
void fts_update_fw_ver(struct fts_ts_data *data)
{
	...
	err = fts_flash_i2c_read(client, &reg_addr, 1, &data->fw_ver[0], 1);
	if (err < 0)
		dev_err(&client->dev, "fw major version read failed");

	reg_addr = FTS_REG_FW_MIN_VER;
	err = fts_flash_i2c_read(client, &reg_addr, 1, &data->fw_ver[1], 1);
	if (err < 0)
		dev_err(&client->dev, "fw minor version read failed");

	reg_addr = FTS_REG_FW_SUB_MIN_VER;
	err = fts_flash_i2c_read(client, &reg_addr, 1, &data->fw_ver[2], 1);
	if (err < 0)
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
	...
	*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		fts_flash_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		msleep(5);
		reg_val[0] = reg_val[1] = 0x00;
		i_ret = fts_flash_i2c_read(client, auc_i2c_write_buf, 0, reg_val, 2);
		if (0 != reg_val[0]) 
		{
			*ucPVendorID = 0;
...
int fts_ctpm_fw_upgrade_ReadProjectCode(struct i2c_client *client, char *pProjectCode)
{
	...
	for (j = 0; j < 33; j++) 
	{
		...
		fts_flash_i2c_read(client, auc_i2c_write_buf, 4, pProjectCode+j, 1);
		if (*(pProjectCode+j) == '\0')
			break;
	}
...
int fts_6x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	fts_flash_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
	if (reg_val[0] != bt_ecc) 
	{
...
int fts_6336GU_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2) 
		{
			FTS_DBG("[FTS] Step 3: GET CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x  FID1 = 0x%x,FID2 = 0x%x\n",
				reg_val[0], reg_val[1],fts_updateinfo_curr.upgrade_id_1,fts_updateinfo_curr.upgrade_id_2);
			break;
		} 
		else 
		{
			FTS_DBG("[FTS] Step 3: GET CTPM ID FAIL ,ID1 = 0x%x,ID2 = 0x%x  FID1 = 0x%x,FID2 = 0x%x\n",
				reg_val[0], reg_val[1],fts_updateinfo_curr.upgrade_id_1,fts_updateinfo_curr.upgrade_id_2);
			dev_err(&client->dev, "[FTS] Step 3: GET CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
		}
	...
	for (j = 0; j < packet_number; j++) 
	{
		...
		for(i = 0;i < 30;i++)
		{
			...
			if(0xb0 == (reg_val[0] & 0xf0) && (0x03 + (j % 0x0ffd)) == (((reg_val[0] & 0x0f) << 8) |reg_val[1]))
			{
				FTS_DBG("[FTS] write a block data finished conut %d\n",i);
				break;
			}
			msleep(1);
		}
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < 15; i++) 
	{
		auc_i2c_write_buf[0] = 0x6a;
		reg_val[0] = reg_val[1] = 0x00;
		fts_flash_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
...
int  fts_8606_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	i_ret =fts_flash_i2c_read(client, auc_i2c_write_buf, 1, reg_val_id, 1);
	if(dw_lenth == 0)
	{
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	fts_flash_i2c_read(client, buf, 4, buf, FTS_SETTING_BUF_LEN);

...
// PullDisable = 0,
    ...
    #define VCC_NAME  "act_ldo8"
    #define VCC_VOL    3300000
#else
    #error "define regulator vcc & vol name here!"
#endif
/* gpio interface define */
#if defined(CONFIG_LIDA_MACH_X5) || defined(CONFIG_LIDA_MACH_X7II)
    #define GPIO_CTP_RST_PIN     RK30_PIN0_PA0
    #define GPIO_CTP_EINT_PIN    RK30_PIN0_PA3
    // #define GPIO_CTP_EINT_PIN_M_EINT  
    // #define GPIO_CTP_RST_PIN_M_GPIO 
#endif


#define GPIO_DIR_OUT         0
#define GPIO_DIR_IN          1
#define GPIO_OUT_ZERO        GPIO_LOW
#define GPIO_OUT_ONE         GPIO_HIGH
#define GPIO_PULL_DISABLE    PullDisable 
#define GPIO_PULL_UP         GPIOPullUp 
#define GPIO_PULL_DOWN       GPIOPullDownl
// #define mt_set_gpio_mode(gpio,mode)    iomux_set_gpio_mode(iomux_gpio_to_mode(gpio))
#define mt_set_gpio_mode(gpio,mode)    iomux_set(mode)
#define mt_set_gpio_dir(gpio,dir,level)     do{                                 \
                                               if(dir == GPIO_DIR_OUT)          \
                                                  gpio_direction_output(gpio,level) ; \
                                               else                                 \
                                                   gpio_direction_input(gpio); \
                                            }while(0)
#define mt_set_gpio_out(gpio,level)     gpio_set_value(gpio,level)
#define mt_set_gpio_pull_enable(gpio,en)
#define mt_set_gpio_pull_select(gpio,pull) gpio_pull_updown(gpio,pull)
/*******************************************************************************
...
extern int fts_flash_i2c_read(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen);
#if FT_ESD_PROTECT

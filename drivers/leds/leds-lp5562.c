                                        ...
                                        const u8 *data, size_t size)
{
        ...
        for (i = 0; i < program_size; i++) {
                if (get_led_status == 1 && i == 1)
                    lp55xx_write(chip, addr[idx] + i, pattern[i] & led_value);
                else
...
/*
int bright_level_high[] = {
    0,   20,  40,  60,  80,
    100, 120, 140, 160, 180,
    200, 220, 240, 
};

int bright_level_low[] = {
    240, 220, 200, 180, 160, 
    140, 120, 100, 80,  60, 
    40,  20,  0
};
*/
int lp5562_brightness[];

extern int get_lamp_brightness;
extern int get_led_status;

static int set_led_brightness()
{
    int i, j;
   
    i = get_lamp_brightness / 20;

    for (j = 0; j < (i+1); j++)
    {
        lp5562_brightness[j] = j * 20;
        //printk("lipf_debug:  >>>>>>  led_brightness  (%d)  (%d)\n", lp5562_brightness[j], j);
    }
    lp5562_brightness[i+1] = get_lamp_brightness;
...
static void lp5562_led_brightness_work(struct work_struct *work)
{
        ...
        //printk("lipf_debug_LP5562: %s enter!!!\n", __FUNCTION__);
        if (old_brightness != get_lamp_brightness) {
            set_led_brightness();
            old_brightness = get_lamp_brightness;
        }
        j = get_lamp_brightness / 20;

        mutex_lock(&chip->lock);
        //lp55xx_write(chip, addr[led->chan_nr], led->brightness);
        if (/*axp_usb_det()*/get_led_status == 1) { //axp_usb_det() judge usb is in or out
            for (i=0; i<(j+1)/*i<13*/; i++) 
            {
                if (i == 3)
                    lp5562_brightness[i] = 60;

                lp55xx_write(chip, LP5562_REG_R_PWM, lp5562_brightness[i]);
                msleep(100);
                //printk("lipf_debug:  >>>> set_brightness  is (%d)1111 \n", lp5562_brightness[i]);
                if(get_led_status != 1)
                    break;
            }
            msleep(1000);

            for (i=(j+1); i>=0/*i<13*/;  i--) 
            {
                if (i == 3)
                    lp5562_brightness[i] = 60;
                ...
                lp55xx_write(chip, LP5562_REG_R_PWM, lp5562_brightness[i]);
                msleep(100);
                //printk("lipf_debug:  >>>> set_brightness  is (%d)2222 \n", lp5562_brightness[i]);
                if(get_led_status != 1)
                    break;
            }
            msleep(1000);
        } else if (get_led_status == 0) {
            lp55xx_write(chip, LP5562_REG_R_PWM, get_lamp_brightness);
        } else {
            lp55xx_write(chip, LP5562_REG_R_PWM, 0);
        }
        mutex_unlock(&chip->lock);
...
static ssize_t lp5562_brightness_show(struct device *dev, 
...
static ssize_t lp5562_brightness_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (count == size) {
		ret = count;
                ...
                led->led_brightness = state;

                led_value = led->led_brightness;
	}

	return ret;
}

static ssize_t lp5562_status_show(struct device *dev, 
		struct device_attribute *attr, char *buf)
{
	struct lp55xx_led *led = i2c_get_clientdata(to_i2c_client(dev));
...
static ssize_t lp5562_status_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct lp55xx_led *led = i2c_get_clientdata(to_i2c_client(dev));
	ssize_t ret = -EINVAL;
	...
	if (count == size) {
		ret = count;
                led->led_status = state;
                get_led_status = state;
	}

	return ret;
}

static DEVICE_ATTR(led_brightness, 0664, lp5562_brightness_show, lp5562_brightness_store);
static DEVICE_ATTR(led_status, 0664, lp5562_status_show, lp5562_status_store);
                        ...
                        const struct i2c_device_id *id)
{
	...
	if (!pdata) {
        ...
        //printk("lipf_debug:  >>>> get_brightness1111  is (%d) <<<<<<< \n", get_lamp_brightness);
        return 0;
...
int set_sdcard_power()
{
    struct regulator *ldo_8;
    uint8_t val;

    ldo_8 = regulator_get(NULL, "axp22_dldo4");
    if(ldo_8 == NULL || IS_ERR(ldo_8)) {
        printk("sdcard power dldo4 set error %s %d\n", __func__, __LINE__);
        return -1;
    }

    axp_read(&axp->dev, 0x18, &val);
    printk("lipf_debug: read AXP 0x18 reg value is (%x)!\n", val);

    regulator_set_voltage(ldo_8, 3000000, 3000000);
    regulator_enable(ldo_8);
    regulator_put(ldo_8);

    axp_read(&axp->dev, 0x18, &val);
    printk("lipf_debug: 1111  read AXP 0x18 reg value is (%x)!\n", val);

    return 0;
}
#endif

static int __init lp5562_init(void)
{
	int ret;
	...
	ret = i2c_add_driver(&lp5562_driver);
	if (ret < 0)
		printk(KERN_ALERT "Adding lp5562 driver failed\n");

	return ret;
}

static void __exit lp5562_exit(void)
{
	i2c_del_driver(&lp5562_driver);
}

module_init(lp5562_init);
module_exit(lp5562_exit);

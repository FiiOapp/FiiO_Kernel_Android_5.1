...
DPCHAR fpga_enabled=0; 
DPULONG fpga_silsig=0x0; 
...
bool get_fpga_status()
{
    return fpga_enabled? true:false;
...
void dp_perform_action (void)
{
    ...
    if (error_code == DPE_SUCCESS)
    {
        #ifdef ENABLE_DISPLAY
        dp_display_text("\r\nAction code "); 
        dp_display_value(Action_code,HEX);
        #endif

    ...
    switch(host->host_dev_id)
    {
        ...
        {            
            ...
            if(irq_pflag){
                ...
                cdetect = cdetect1;
                /* printk("cdetect %d loop %d delay %d from irq \n",cdetect,i,delay); */
                if(host->det_pin.enable)
                    cdetect = cdetect >=3? 1:0;
                else
                    cdetect = cdetect >=3? 0:1;

                printk("cdetect %d cdetect1 %d loop %d delay %d from irq \n",cdetect,cdetect1,i,delay);
            }
            else{
                delay=10;
                ...
                cdetect = cdetect1;          
                /* printk("cdetect %d loop %d delay %d irq %b\n",cdetect,i,delay,irq_pflag); */
                if(host->det_pin.enable)
                    cdetect = cdetect >=2? 1:0;
                else
                    cdetect = cdetect >=2? 0:1;
            }
...
static void rk29_sdmmc_isr_work(struct work_struct *work)
{
	int ret;
    struct rk29_sdmmc *host =  container_of(work, struct rk29_sdmmc, work_isr.work);
    ...
    if(present != present_old)
    {
    	...
    	{
    	    set_bit(RK29_SDMMC_CARD_PRESENT, &host->flags);
    		schedule_delayed_work(&host->work, msecs_to_jiffies(100));
    	}
    	else
    	{
    	    clear_bit(RK29_SDMMC_CARD_PRESENT, &host->flags);
...
#endif	
    enable_irq(host->gpio_irq);
}
#endif


static irqreturn_t det_keys_isr_wq(int irq, void *dev_id)
{
	struct rk29_sdmmc *host = dev_id;

    disable_irq_nosync(irq);
    printk("%s %d\n",__FUNCTION__,__LINE__);
    schedule_delayed_work(&host->work_isr, msecs_to_jiffies(20));
	return IRQ_HANDLED;
}
static irqreturn_t det_keys_isr(int irq, void *dev_id)
{
...
static int rk29_sdmmc_probe(struct platform_device *pdev)
{
   ...
   if((RK29_CTRL_SDMMC_ID == host->host_dev_id)|| (RK29_SDMMC_EMMC_ID == host->host_dev_id)) 
    ...
    if((RK29_CTRL_SDMMC_ID == host->host_dev_id) && (INVALID_GPIO != host->det_pin.io))
    {
        ...
        INIT_DELAYED_WORK(&host->work_isr, rk29_sdmmc_isr_work);
        ...
        /* ret = request_irq(host->gpio_irq, det_keys_isr, */
						...
						/* "sd_detect", */
						/* host); */
        ret = request_irq(host->gpio_irq, det_keys_isr_wq,
					    level_value?IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING,
					    "sd_detect",
					    host);
		if(ret < 0) {

...
static struct class *suspend_class;
...
static void suspend(struct work_struct *work)
{
	...
	char *sys_status[2]   = { "SYS_STATUS=SYSPEND", NULL };
        char **suspend_envp = NULL;
	...
	if (has_wake_lock(WAKE_LOCK_SUSPEND)) {
        ...
        suspend_envp = sys_status;
        if (suspend_envp) {
                kobject_uevent_env(&suspend_dev->kobj, KOBJ_CHANGE, suspend_envp);
                pr_info("suspend: start suspend_envp !\n");
        }
...
static ssize_t suspend_status_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
        long has_lock;
...
static DEVICE_ATTR(suspend_status, S_IRUGO | S_IWUSR, suspend_status_show, NULL);

static int suspend_class_init(void)
{
        if (!suspend_class) {
                suspend_class = class_create(THIS_MODULE, "suspend_class");
		if (IS_ERR(suspend_class))
			return PTR_ERR(suspend_class);
        }
...
static int __init wakelocks_init(void)
{
	...
	proc_create("wakelocks", S_IRUGO, NULL, &wakelock_stats_fops);
        ...
        if (!suspend_class) {
                ret = suspend_class_init();
		if (ret < 0)
			return ret;
        }

        suspend_dev = device_create(suspend_class, NULL, MKDEV(0, 0), NULL, "sys_status");
	if (IS_ERR(suspend_dev))
		return PTR_ERR(suspend_dev);

	ret = device_create_file(suspend_dev, &dev_attr_suspend_status);
	if (ret < 0)
		goto err_create_suspend_file;

	return 0;

err_create_suspend_file:
        device_destroy(suspend_class, MKDEV(0, 0));
...
static void  __exit wakelocks_exit(void)
{
	...
	wake_lock_destroy(&deleted_wake_locks);
        ...
        device_remove_file(suspend_dev, &dev_attr_suspend_status);
        device_destroy(suspend_class, MKDEV(0, 0));
}

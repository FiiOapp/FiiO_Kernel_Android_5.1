						...
						unsigned int fmt)
{
	...
	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
		case SND_SOC_DAIFMT_RIGHT_J:
			...
			tx_ctl &= ~I2S_FIRST_BIT_MASK;    // default MSB
			tx_ctl |= I2S_FIRST_BIT_MSB;
			break;
		...
		case SND_SOC_DAIFMT_I2S:
			...
			tx_ctl |= I2S_FIRST_BIT_MSB;
			break;
		case SND_SOC_DAIFMT_MSB_LEFT_J :
			tx_ctl &= ~I2S_BUS_MODE_MASK;    //I2S Bus Mode
			tx_ctl |= I2S_BUS_MODE_LSJM;
			tx_ctl &= ~I2S_FIRST_BIT_MASK;    // 
			tx_ctl |= I2S_FIRST_BIT_MSB;
			break;
		case SND_SOC_DAIFMT_LSB_LEFT_J :
			tx_ctl &= ~I2S_BUS_MODE_MASK;    //I2S Bus Mode
			tx_ctl |= I2S_BUS_MODE_LSJM;
			tx_ctl &= ~I2S_FIRST_BIT_MASK;    // 
			tx_ctl |= I2S_FIRST_BIT_LSB;
			break;
		default:
	...
	printk("Enter::%s----%d, I2S_TXCR=0x%X\n",__FUNCTION__,__LINE__,tx_ctl);
	...
	int clk_id, unsigned int freq, int dir)
{
    ...
    printk(" i2s Clock name %s  set freq %u current freq %lu \n",i2s->iis_clk->name,freq,i2s->iis_clk->rate);
...
{	
    ...
    switch(dai->id) {
        ...
        case 1:
            ...
            // X5: gpio1_c2 used for MSP430 reset.

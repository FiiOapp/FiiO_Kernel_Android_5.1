/*
 * rk29_es9018.c  --  SoC audio for rockchip
 *
 * Driver for rockchip es9018 audio
 *
	...
	else
	{
        ...
        if(isI2sFormat_LJF((params_format(params)))){
            ...
            if(params_format(params) == SNDRV_PCM_FORMAT_SOP)
		        ...
		        ret = snd_soc_dai_set_fmt(cpu_dai, SND_SOC_DAIFMT_LSB_LEFT_J ); //dsd MSB first
...
/*
 * Logic for a es9018 as connected on a rockchip board.
 */
static int rk29_es9018_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
	...
	printk("Enter rk29_es9018_init!\n");  
	DBG("Enter::%s----%d\n",__FUNCTION__,__LINE__);

	ret = snd_soc_dai_set_sysclk(codec_dai, 0,
			11289600, SND_SOC_CLOCK_IN);
	if (ret < 0) {
		printk(KERN_ERR "Failed to set es9018 SYSCLK: %d\n", ret);
		return ret;
	}
...
static struct snd_soc_dai_link rk29_dai = {
	.name = "ES9018",
	.stream_name = "ES9018 PCM",
	.codec_name = "ES9018.3-0048",
